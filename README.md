...
Note:
An OpenLiberty config file with the name of postgresql-ds.xml is provided specifically for testing with postgress.
This is config file is included in OpenLiberty's main server.xml config file.
...
(*) Items that needed to be done that are not currently documented, but needed to be done.
